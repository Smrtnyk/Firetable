...
import { getDrinkCardsPath, getDrinkCardPath } from "./paths.js";
...
export async function addDrinkCard(
    ...
    const drinkCardsCollection = collection(
        firestore,
        getDrinkCardsPath(organisationId, propertyId),
    );

    const docRef = await addDoc(drinkCardsCollection, {
        ...card,
