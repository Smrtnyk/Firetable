...
import type { DrinkCardItem, DrinkCardSection, InventoryItemDoc } from "@firetable/types";
import type { SortableEvent } from "vue-draggable-plus";
import { getSectionTemplates } from "./section-templates.js";
...
const filteredInventoryItems = computed(function () {
    return props.inventoryItems.filter(function (item) {
        return item.mainCategory === props.section.category;
    });
});

const sectionTemplates = computed(() => getSectionTemplates(t));
...
const showItemSelectionDialog = ref(false);

function handleAddItem(inventoryItem: InventoryItemDoc): void {
    const template = sectionTemplates.value.find(({ id }) => id === props.section.template);
    const servingSize = template
...
function handleSpecialPriceUpdate(
    index: number,
    { field, value }: { field: "amount" | "description" | "label"; value: number | string },
): void {
    const newItems = Array.from(props.section.items);
    ...
    <div class="DrinkCardSectionItems q-gutter-y-sm">
        <div class="row items-center justify-end q-my-md">
            <q-btn
                ...
                :disable="filteredInventoryItems.length === 0"
                @click="showItemSelectionDialog = true"
            />
        </div>
