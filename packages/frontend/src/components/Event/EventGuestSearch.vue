...
function onSearchUpdate(value: string): void {
    if (selectedItem.value && selectedItem.value.label === value) {
        return;
    }
    ...
    const normalizedSearchVal = value.toLowerCase().trim();
    const searchTokens = normalizedSearchVal.split(/\s+/).filter((str) => str.length > 0);

    if (searchTokens.length === 0) {
        // If search input is all spaces or effectively empty after tokenizing
        emit("clear");
        return;
    }

    const resultsBasedOnlyOnSearchTerm = props.allReservedTables.filter(function (reservation) {
        const guestName = reservation.guestName.toLowerCase();
        const guestNameTokens = guestName.split(/\s+/);

        if (guestNameTokens.length === 0 && searchTokens.length > 0) {
            return false;
        }

        return searchTokens.some(function (token) {
            return guestNameTokens.some(function (nameToken) {
                return nameToken.includes(token);
            });
        });
    ...
    emit("found", resultsBasedOnlyOnSearchTerm);
}
