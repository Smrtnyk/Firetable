...
export async function onPropertyDeletedCleanEvents(
    ...
    try {
        ...
        const eventsSnapshot = await db.collection(Collection.EVENTS)
            .where("propertyId", "==", propertyId)
            .get();

        if (eventsSnapshot.empty) {
            functions.logger.info(`No events associated with property ${propertyId}. Exiting function.`);
            return;
        }
        ...
        functions.logger.info(`Successfully deleted all events and their subcollections associated with property ${propertyId}.`);

    } catch (error) {
        functions.logger.error("Error deleting events associated with property:", error);
        throw new Error(`Failed to delete events and their subcollections associated with property ${propertyId}`);
    }
}
