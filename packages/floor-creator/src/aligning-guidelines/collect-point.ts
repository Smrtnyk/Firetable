...
type VerticalPoint = { x: number; y1: number; y2: number };
export function collectVerticalPoint(props: CollectPointProps): VerticalPoint[] {
...
type HorizontalPoint = { y: number; x1: number; x2: number };

export function collectHorizontalPoint(props: CollectPointProps): HorizontalPoint[] {
    const { activeObject, isScale, index, point, list } = props;
...
type DistanceList = { dis: number; arr: Point[] };

function getDistanceList(point: Point, list: Point[], type: "x" | "y"): DistanceList {
    let dis = Infinity;
    let arr: Point[] = [];
