...
describe("getDistanceList", () => {
    it("returns the distance list", () => {
        ...
        const list = [new Point(2, 3), new Point(-2, -3), new Point(3, 3), new Point(4, 4)];
...
describe("getPointMap", () => {
    it("returns the pointMap", () => {
        ...
        expect([pointMap.tl, pointMap.tr, pointMap.br, pointMap.bl]).toEqual(coords);
        const arr = [];
...
function rnd(start: number, end?: number): number {
    if (end === undefined) {
        // eslint-disable-next-line no-param-reassign -- patched from fabric
        end = start;
        // eslint-disable-next-line no-param-reassign -- patched from fabric
        start = 0;
    }
