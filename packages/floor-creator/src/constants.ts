export const RESOLUTION = 15;
export const ELEMENT_DEFAULT_STROKE_WIDTH = 0.5;

export const TABLE_WIDTH = RESOLUTION * 2 - ELEMENT_DEFAULT_STROKE_WIDTH;
export const TABLE_HEIGHT = RESOLUTION * 2 - ELEMENT_DEFAULT_STROKE_WIDTH;

export const FLOOR_DEFAULT_WIDTH = 60 * 15;
export const FLOOR_DEFAULT_HEIGHT = 70 * 15;
export const MAX_FLOOR_WIDTH = RESOLUTION * 70;
export const MAX_FLOOR_HEIGHT = RESOLUTION * 100;
export const FONT_SIZE = 15;
export const DEFAULT_FONT = "Product Sans, Roboto, sans-serif";
export const CANVAS_BG_COLOR = "#333";
export const TABLE_TEXT_FILL_COLOR = "#fff";
export const ELEMENT_DEFAULT_FILL_COLOR = "#444";
export const ELEMENT_DEFAULT_STROKE_COLOR = "#000";

export const DEFAULT_ZOOM = 1;
export const ZOOM_INCREMENT = 0.3;
